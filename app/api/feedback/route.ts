...
export async function PATCH(req: NextRequest) {
  ...
  let oid: ObjectId;
  try { oid = new ObjectId(id); } catch { return NextResponse.json({ ok: false, error: 'invalid id' }, { status: 400 }); }

  if (action === 'vote') {
    const item = await db.collection('feedback').findOne({ _id: oid });
    if (!item) return NextResponse.json({ ok: false, error: 'not found' }, { status: 404 });

    const already = (item.votedBy ?? []).includes(session.email);
    if (already) {
      await db.collection('feedback').updateOne({ _id: oid }, {
        $inc: { votes: -1 },
        $pull: { votedBy: session.email } as any,
      });
    } else {
      await db.collection('feedback').updateOne({ _id: oid }, {
        $inc: { votes: 1 },
        $addToSet: { votedBy: session.email },
      });
    }
    return NextResponse.json({ ok: true, voted: !already });
  }

  if (action === 'status' && ['owner', 'admin'].includes(session.role)) {
